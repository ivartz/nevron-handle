...
resource "openstack_compute_servergroup_v2" "group-tiny" {
  name     = "anti-affinity"
...
resource "openstack_compute_instance_v2" "tiny" {
  count = 8
  name = format("tiny-%02d", count.index + 1)
  ...
  scheduler_hints {
    group = openstack_compute_servergroup_v2.group-tiny.id
  }
